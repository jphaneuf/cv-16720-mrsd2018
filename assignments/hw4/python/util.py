...
###Hacked util.py for plot settings in plot_matches for rotation test
#/home/joe/anaconda2/lib/python2.7/site-packages/skimage/feature
import numpy as np
...
def plot_matches(ax, image1, image2, keypoints1, keypoints2, matches,
    ...
    for i in range(matches.shape[0]):
        ...
        color = np.random.choice(['b' , 'g' , 'r' , 'c' , 'm' , 'y' ] , 1 )[0]
        lines = ax.plot((keypoints1[idx1, 1], keypoints2[idx2, 1] + offset[1]),
                (keypoints1[idx1, 0], keypoints2[idx2, 0]),
                '-', color=color , linewidth = 1 , alpha = 1 ) 
        #ok = 1 if raw_input ( ' ok ?' ) == 'y' else 0
        #data.append ( ok )
        #plt.show()
        #lines.pop(0).remove()
