...
def triangulate( P1, pts1_in , P2 , pts2_in ):
  ...
  for i in range ( r ):
    ...
    sq_errors1.append ( norm ( p1 - p1hat [ 0 : 2 ].T  ) ** 2 ) 
    sq_errors2.append ( norm ( p2 - p2hat [ 0 : 2 ].T  ) ** 2 ) 
  print sum ( sq_errors1 )
  print sum ( sq_errors2 )
  err = sum ( sq_errors1 )  + sum ( sq_errors1 )
  print err
  P = np.array   ( P )
